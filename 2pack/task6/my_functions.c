...
status_code reading(FILE *input, Student **students, size_t *count) {
    ...
    while (fscanf(input, "%u %99s %99s %99s %hhu %hhu %hhu %hhu %hhu", &id, name, lastname, group, &grades[0], &grades[1], &grades[2], &grades[3], &grades[4]) == 9) {
        temp_count++;
    }
    ...
    while (fscanf(input, "%u %99s %99s %99s %hhu %hhu %hhu %hhu %hhu", &id, name, lastname, group, &grades[0], &grades[1], &grades[2], &grades[3], &grades[4]) == 9) {
        ...
        for (int j = 0; name[j] != '\0'; j++) {
            if (!isalpha(name[j])) { name_ok = false; break; }
        }
        for (int j = 0; lastname[j] != '\0'; j++) {
            if (!isalpha(lastname[j])) { lastname_ok = false; break; }
        }
        if (!name_ok || !lastname_ok) continue;

        bool grades_valid = true;
        for (int j = 0; j < NUM_EXAMS; j++) {
            if (grades[j] > 5) { grades_valid = false; break; }
        }
        if (!grades_valid) continue;
        ...
        if (!temp_students[num_loaded].marks) {
            for (size_t k = 0; k < num_loaded; k++) free(temp_students[k].marks);
            free(temp_students);
            fclose(input);
            return ALLOCATION_ERROR;
        }
    ...
    *students = realloc(temp_students, num_loaded * sizeof(Student));
    if (!(*students) && num_loaded > 0) *students = temp_students;
    *count = num_loaded;
